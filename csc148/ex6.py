...
class Tree:
    ...
    def get_path(self, value) -> list:
        """
        return a list of values of nodes from the root to node with value given.
        """
        if not self.contains(value):
            return []
        return [self.value] + sum([child.get_path(value)
                                   for child in self.children], [])
    ...
    def get_nodes_connecting(self, value1: Any, value2: Any) -> list:
        ...
        if not self.contains(value1) or not self.contains(value2):
            ...
            return [value1]
        path1 = self.get_path(value1)
        path1.reverse()
        path2 = self.get_path(value2)
        for i in range(len(path1)):
            if path1[i] in path2:
                return path1[:i] + path2[path2.index(path1[i]):]
        return []
